...
let allTrs = table.children[0].getElementsByTagName('tr');


for(let i=0; i<allTrs.length; i++){

    let allRdsPerRow = allTrs[i].getElementsByTagName('td');

    for(let j=0; j<allRdsPerRow.length; j++){

        if(i === j){

            allRdsPerRow[j].setAttribute('class','red'); 
        }

        if(i === (allRdsPerRow.length-1)-j ){

            allRdsPerRow[j].setAttribute('class','coral'); 
        }
    }
}
